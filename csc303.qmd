...
Suppose statisticians think too simply.  Maybe they make graphs like this @fig-vitamin-supplement:
...
The statisticians have found that `supplement` and `becnh` are *associated*.  But they really want to know whether `supplement` is part of what *causes* `bench`.
...
Well, it is clearly associated with `supplment`, as shown here in @tbl-supplement-workout:
...
So `workout` is associated with `supplment`, and associated with `bench`.  If it were actually part of what *causes* `bench`, then it would be a confounding factor.
